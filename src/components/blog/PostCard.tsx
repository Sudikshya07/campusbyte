...
export default function PostCard({ post, variant = "default" }: PostCardProps) {
    if (variant === "featured") {
        return (
            <Link href={`/blogs/${post.slug}`} className="group relative block w-full h-[400px] overflow-hidden rounded-2xl">
                <Image
                    src={post.imageUrl}
                    alt={post.title}
                    fill
    ...
    if (variant === "compact") {
        return (
            <Link href={`/blogs/${post.slug}`} className="group flex gap-4 items-start">
